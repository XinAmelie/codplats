...
const editorHeaders = ref(null)

const initEditor = () => {
  editorHeaders.value = monaco.editor.create(document.getElementById('codeEditBoxHeaders'), {
    value: '{}', // 编辑器初始显示文字
  ...
  editorHeaders.value.onDidChangeModelContent((val) => {
    // console.log(val.changes[0].text)
    const modifyContent = toRaw(editorHeaders.value).getValue()
...
onMounted(() => {
  initEditor()
  setTimeout(() => {
    toRaw(editorHeaders.value).setValue(content)
    editorHeaders.value.getAction('editor.action.formatDocument').run() // 自动格式化代
