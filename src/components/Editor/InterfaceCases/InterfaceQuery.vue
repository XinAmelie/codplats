...
console.log(typeof lastTestjson)

// 获取数据
const jsdata = ref('')
const a = ref('')
axios.get('/Jsondata/ManoEdit.json/')
  .then(res => {
    jsdata.value = res
    a.value = JSON.stringify(toRaw(jsdata.value.data))
  })

const editor = ref(null)

const initEditor = () => {
  editor.value = monaco.editor.create(document.getElementById('codeEditBoxQuery'), {
    value: '{}', // 编辑器初始显示文字
...
onMounted(() => {
  initEditor()
  setTimeout(() => {
    toRaw(editor.value).setValue(a.value)
